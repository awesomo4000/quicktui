...
static void messages(JSContext *ctx) {
    ...
    for(int i=0;i<32&&!host->failed;i++){
...
int quicktui_app_messages(const char *source,size_t length,int headless,const char *example,const MessageEndpoint *endpoint) {
    AppHost host={.endpoint=endpoint};
    ...
    if(headless){
        ...
        if(has_custom_test){
            ...
            for(int i=0;i<10000&&!host.failed&&JS_PromiseState(ctx,test_result)==JS_PROMISE_PENDING;i++){step(ctx,runtime); if(endpoint){struct pollfd fd={.fd=endpoint->wake_fd,.events=POLLIN};poll(&fd,1,1);}}
    ...
    while(!host.quit&&!host.failed&&!interrupted){
        ...
        struct pollfd fds[]={{.fd=STDIN_FILENO,.events=POLLIN},{.fd=wake[0],.events=POLLIN},{.fd=endpoint?endpoint->wake_fd:-1,.events=POLLIN}};
