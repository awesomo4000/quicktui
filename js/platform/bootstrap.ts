...
const process = Object.assign(new EventEmitter(),{env:__host.env,arch:__host.arch,platform:__host.platform,versions:{quickjs:"2026-06-04"},cwd:()=>".",hrtime:Object.assign(()=>{const n=__host.now();return [Math.floor(n/1000),Math.floor(n%1000*1e6)]},{bigint:()=>BigInt(Math.floor(__host.now()*1e6))})});
...
let nextTimer=1;
const timers=new Map<number,{due:number,fn:Function,args:any[]}>();
...
Object.assign(globalThis,{__timers:{
  tick(){const now=__host.now();let count=0;for(const [id,t] of [...timers])if(t.due<=now&&timers.delete(id)){t.fn(...t.args);if(++count===128)break}},
