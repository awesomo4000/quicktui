...
const batch=(work:()=>void)=>adapter?.batch?adapter.batch(work):work();
let native:any;
...
Object.assign(globalThis,{
  ...
  __input(data:ArrayBuffer,time=__host.now()){receivedAt=time;batch(()=>{parser.push(new Uint8Array(data));drainInput();if(inputOverflow){inputOverflow=false;resetInput("overflow")}})},
  ...
  __inspect(){return JSON.stringify({...adapter?.inspect?.(),applicationMounted:mounted,keys:keys.listenerCount("key"),frame:context.frameId})},
  __snapshot(){return new TextDecoder().decode(lib.getCurrentBuffer(native).getRealCharBytes(true))},
});
export function mountApplication(nextAdapter:ApplicationAdapter,options:ApplicationOptions={},shortcuts=false){
if(mounted||stopped)throw new Error("Only one application mount per runtime is supported");
mounted=true;adapter=nextAdapter;demoShortcuts=shortcuts;
appOptions=options;
...
return {root,context,quit:()=>__host.quit(),snapshot:()=>new TextDecoder().decode(lib.getCurrentBuffer(native).getRealCharBytes(true))};
}
