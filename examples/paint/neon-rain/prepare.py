...
def water(c,x,y):
 ...
 return (cyan or pink) and (y>126 or (x<75 or x>176))
...
obj={'format':'termpaint','version':3,'width':256,'height':192,'palette':['#%02x%02x%02x'%c for c in colors],'cycle':{'start':24,'end':31,'stepMs':130,'direction':1,'blend':True},'pixels':''.join(alpha[n] for n in indices)}
(root/'neon-rain.tpaint').write_text(json.dumps(obj,separators=(',',':')))
...
png(root/'neon-rain.png');png(Path('/tmp/neon-rain-preview.png'),3)
print('Neon mask pixels:',sum(mask),'animated:',sum(n>=24 for n in indices),'file bytes:',(root/'neon-rain.tpaint').stat().st_size)
